...
namespace Cinemachine
{
    ...
    public class CinemachineGroupComposer : CinemachineComposer
    {
        ...
        public Vector2 m_GroupFramingSize = new Vector2(0.8f, 0.8f);
        ...
        {
            m_GroupFramingSize.x = Mathf.Max(Epsilon, m_GroupFramingSize.x);
            m_GroupFramingSize.y = Mathf.Max(Epsilon, m_GroupFramingSize.y);
        ...
        {
            float framingSizeX = Mathf.Max(Epsilon, m_GroupFramingSize.x);
            float framingSizeY = Mathf.Max(Epsilon, m_GroupFramingSize.y);
            switch (m_FramingMode)
            {
                case FramingMode.Horizontal:
                    return Mathf.Max(Epsilon, b.size.x )/ (framingSizeX * VcamState.Lens.Aspect);
                case FramingMode.Vertical:
                    return Mathf.Max(Epsilon, b.size.y) / framingSizeY;
                default:
                case FramingMode.HorizontalAndVertical:
                    return Mathf.Max(
                        Mathf.Max(Epsilon, b.size.x) / (framingSizeX * VcamState.Lens.Aspect), 
                        Mathf.Max(Epsilon, b.size.y) / framingSizeY);
            }
        }
    }
}
